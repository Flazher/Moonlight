...
int GetMenuEntriesCount ();
static void AddMenuEntry (char *);
...
MenuEntry *HeaderMenuEntry = NULL;
...
MoonlightDisplayInit ()
{
    ...
    Fill (&Moonlight.DC);
    XSetWindowBackgroundPixmap(Dpy, Moonlight.Root, Moonlight.DC.drawable);
...
EnterNotifyHandler (XEvent *Event)
{
    ...
    if (!MenuIsHidden && Event->xfocus.window != Moonlight.Root && Event->xfocus.window != Menu && Menu)
        ShowMenu ();
...
ShowMenu ()
{
    ...
    if (!Menu)
    {
        AddMenuEntry ("You're welcome");
        AddMenuEntry ("Unfinished dummy WM");
        AddMenuEntry ("This is Moonlight");
        ...
        Menu = XCreateWindow
        (
            Dpy, Moonlight.Root, MousePosition.x, MousePosition.y, MENUWIDTH, GetMenuEntriesCount() * MENUENTRYHEIGHT, 0,
        ...
        MenuDC = CreateDrawingContext (
         Menu, MENUWIDTH, MENUHEIGHT, GetColor (WINBARCOLOR), GetColor (WINBARCOLOR)
        );
        Fill (&MenuDC);
        ...
        #ifdef XFT
        XftDraw *dummy = XftDrawCreate(Dpy, MenuDC.drawable, DefaultVisual(Dpy, DefaultScreen(Dpy)), DefaultColormap(Dpy, DefaultScreen(Dpy)));
        #endif
        MenuEntry *entry;
        
        int i = 0;
        for (entry = HeaderMenuEntry; entry; entry = entry->Next)
        {
            entry->window = XCreateWindow
            (
                Dpy, Menu, MousePosition.x, MousePosition.y + i * MENUENTRYHEIGHT + i*3, MENUWIDTH, MENUENTRYHEIGHT, 0,
                DefaultDepth (Dpy, Moonlight.DefaultScreen), CopyFromParent,
                DefaultVisual (Dpy, Moonlight.DefaultScreen),
                CWOverrideRedirect|CWBackPixmap|CWEventMask|CWCursor, &MenuAttrs
            );

            //DrawingContext lolbljad = CreateDrawingContext (entry->window, MENUWIDTH, MENUHEIGHT, GetColor (MENUCOLOR), GetColor (MENUCOLOR));
            //Fill (&lolbljad);
            #ifdef XFT
            XftDrawStringUtf8 (dummy, &DefaultFG, DefaultXFTFont, 10, 20 + i * 30, entry->title, strlen(entry->title));
            #endif
            //XSetWindowBackgroundPixmap(Dpy, entry->window, lolbljad.drawable);
            i++;
        }
    }
    else
    ...
    MenuEntry *entry;
    for (entry = HeaderMenuEntry; entry; entry = entry->Next)
        (MenuIsHidden ? XMapRaised (Dpy, entry->window) : XUnmapWindow (Dpy, entry->window));
    MenuIsHidden ^= True;
}

static void
AddMenuEntry (char *title)
{
    MenuEntry *entry;
    entry = malloc (sizeof *entry);
    entry->Next = HeaderMenuEntry;
    entry->title = title;
    HeaderMenuEntry = entry;
}
...
GetMenuEntriesCount ()
{
    int count;
    MenuEntry *entry;
    for (entry = HeaderMenuEntry; entry; entry = entry->Next) count++;
    return count;
}
